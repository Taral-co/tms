...
func (r *ticketRepository) Create(ctx context.Context, ticket *db.Ticket) error {
	query := `
		INSERT INTO tickets (id, tenant_id, project_id, number, subject, status, priority, type, source, requester_id, assignee_agent_id, created_at, updated_at)
	...
	_, err := r.db.ExecContext(ctx, query,
		...
		ticket.RequesterID, ticket.AssigneeAgentID)
	if err != nil {
...
func (r *ticketRepository) GetByID(ctx context.Context, tenantID, projectID, ticketID uuid.UUID) (*db.Ticket, error) {
	query := `
		SELECT id, tenant_id, project_id, number, subject, status, priority, type, source, requester_id, assignee_agent_id, created_at, updated_at
		FROM tickets
	...
	err := r.db.QueryRowContext(ctx, query, tenantID, projectID, ticketID).Scan(
		...
		&ticket.Source, &ticket.RequesterID, &ticket.AssigneeAgentID,
